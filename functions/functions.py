...
class HelperFunctions:
    ...
    def query_database(source: Path, query: str) -> str:
    ...
    def get_destf_name(dest: Path, destf: str, time: str) -> str:
...
[-] The .csv file was created successfully and is saved as :
    [dodger_blue1]{Path(output_csv_file).name}""")
        ...
        c.print(f"""[grey66]
[-] The .kml file was created with [dodger_blue1]{count:,} [grey66]records and \
is saved as :
    [dodger_blue1]{Path(output_kml_file).name}""")

        # Show directory where the output .kml file is saved
        c.print(f"""[grey66]
[-] The output file(s) are saved in the [dodger_blue1]\
{Path(output_kml_file).parent}\\ [grey66]directory
""")
...
[-] Do you want to open the .kml file now? \[y/n] >> """)
    ...
    Opening the .kml file with Google Earth...""")
            ...
            # If the user enteres anything other than "y" exit program
            # Print closing message on the screen
            c.print(f"\n{closing}")
